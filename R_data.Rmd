...
HI_move_in <- hi_dc_flows %>% filter(FULL1_NAME %like% 'Hawaii') %>%
...
DC_move_in <- hi_dc_flows %>% filter(FULL1_NAME %like% 'District of Columbia') %>%
  filter(!is.na(GEOID2), variable == "MOVEDIN") %>% 
  slice_max(n = 50, order_by = estimate) %>% 
  mutate(
    width = estimate / 500,
...
GA_move_out <- ga_ct_flows %>% filter(FULL1_NAME %like% 'Georgia') %>% 
...
CT_move_out <- ga_ct_flows %>% filter(FULL1_NAME %like% 'Connecticut') %>%
